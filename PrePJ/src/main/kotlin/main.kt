...
object Example : Automaton {
    ...
    override val finalStates = setOf(
        2, 5, 6, 14, 15, 21, 22, 26, 27, 30, 31, 35, 36, 42, 44, 45, 46, 47, 48, 49, 50, 52,
...
fun main(args: Array<String>) {
    for (file in args) {
        ...
        val city: City? = Scanner(Example, inputStream).start()
        if (city != null) {
            ...
            for (road in city.roads) {
                ...
                for (point in road.points) {
                    val coordinate = LngLatAlt(point.x, point.y)
                    geometry.coordinates.add(coordinate)
                }
                feature.geometry = geometry
                collection.add(feature)
            }
            val json = ObjectMapper().writeValueAsString(collection)
            File("examples/out/" + f.name.replace(".txt", ".geojson")).writeText(json)
        }
        inputStream.close()
    }
}
...
/*fun main() {
    val roadsFile = File("origin/roads.geojson").inputStream()
    val roadsCollection: FeatureCollection = ObjectMapper().readValue(roadsFile, FeatureCollection::class.java)
    val roads = mutableListOf<Road>()
    for (feature in roadsCollection.features) {
        val f = feature.geometry as MultiLineString
        val points = mutableListOf<models.Point>()
        for (coordinate in f.coordinates) {
            for(coord in coordinate) {
                val c = coord as LngLatAlt
                val point = models.Point(c.longitude, c.latitude)
                points.add(point)
            }
        }
        val road = Road(feature.properties["ID"]!!.toString(), points)
        roads.add(road)
    }
    roadsFile.close()

    val estatesFile = File("origin/estates.geojson").inputStream()
    val estatesCollection: FeatureCollection = ObjectMapper().readValue(estatesFile, FeatureCollection::class.java)
    val estates = mutableListOf<Estate>()
    for (feature in estatesCollection.features) {
        val f = feature.geometry as MultiPolygon
        val points = mutableListOf<models.Point>()
        for (coordinate in f.coordinates) {
            for(coord in coordinate) {
                for (coord1 in coord) {
                    val c = coord1 as LngLatAlt
                    val point = models.Point(c.longitude, c.latitude)
                    points.add(point)
                }
            }
        }
        val name = feature.properties["PC_MID"]!!.toString() + feature.properties["SIFKO"]!!.toString() +
                feature.properties["STA_STEV"]!!.toString()
        val estate = Estate(name, points)
        estates.add(estate)
    }
    estatesFile.close()

    val city = City("Maribor", roads, estates)
    File("origin/Maribor_backup.txt").writeText(city.toString())
}*/